...
include files : service-url.inc/front-service-url.inc/chatbot-service-url.inc </br>
inc파일은 무중단 배포를 위한 파일로 jenkins pipeline에서 포트 확인을 통해 수정됨 </br>
ex) set $service_url http://localhost:8080 -> set $service_url http://localhost:8081  </br>
...
jenkins/workspace 하위에 config dir 생성 후 도커컴포즈, .env 등 배포에 필요한 파일 보관 </br>
running port와 available port로 구분해 각각의 포트에 해당하는 docker-compose 작성
...
treatment.json : 비급여 항목 데이터를 아리랑 형태소 분석기를 사용해 사전 생성
![image.png](./image.png)
